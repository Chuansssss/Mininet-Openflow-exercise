...
class RouterExercise (object):
  ...
  def __init__ (self, connection):
    ...
    self.routingTable = {"10.0.1.0/24" :['10.0.1.100', 's1-eth1', '10.0.1.1', 1,'00:00:00:00:00:01'],
  ...
  def ipv4pkt_send(self, packet, packet_in):
      subnetMatch = 0
      log.debug("ipv4 packet with src ip %r dst ip %r"%(packet.payload.srcip,packet.payload.dstip) )
      for subnetID in self.routingTable.keys():
          if packet.payload.dstip.inNetwork(subnetID):
              subnetMatch = subnetID
              break
      if subnetMatch == 0:
          log.debug("this packet is unreachable")
          ...
          msg.data = packet_in
          msg.actions.append(of.ofp_action_dl_addr.set_dst(EthAddr(self.routingTable[subnetMatch][4])))
          msg.actions.append(of.ofp_action_dl_addr.set_src(packet.dst))
          msg.actions.append(of.ofp_action_output(port = self.routingTable[subnetMatch][3]))
          self.connection.send(msg)
      ...
      if packet.type == pkt.ethernet.ARP_TYPE: #if the packet's type is ARP
          ...
          if ipPkt.protocol == pkt.ipv4.ICMP_PROTOCOL: #judge whether it is a ICMP
              ...
              if icmpPacket.type == pkt.TYPE_ECHO_REQUEST:
                  ...
                  # see whether the ip is reachable
                  dstIpMatch = 0

                  # match the ip in routingTable
                  for subnetID in self.routingTable.keys():
                      if ipDstAddr.inNetwork(subnetID):
                          dstIpMatch = subnetID
                          break

                  #if the ip is unreachabe
                  if dstIpMatch == 0:
                      log.debug("ip address is unreachable")
                      self.unreachable_send(packet, packet_in)

                  #if there is a match for the ip, reachable
                  else:
                      log.debug("The subnet of ICMP's ip address is in the routing table")
                      if str(ipDstAddr) == self.routingTable[dstIpMatch][2]:
                          self.reachable_send(packet, packet_in)
                      
                      elif str(ipDstAddr) == self.routingTable[dstIpMatch][0]:
      
                           packet.src = packet.dst
                           packet.dst = EthAddr(self.routingTable[dstIpMatch][4])
                           msg = of.ofp_packet_out()
                           msg.data = packet.pack()

                           action = of.ofp_action_output(port = self.routingTable[dstIpMatch][3])
                           msg.actions.append(action)
                           self.connection.send(msg)
                  ...
                  #judge whether the ip can be found in routingTable
                  for subnetID2 in self.routingTable.keys():
                      if (packet.payload.dstip).inNetwork(subnetID2):
                          dstIpMatch2 = subnetID2
                          break

                  packet.src = packet.dst
                  packet.dst = EthAddr(self.routingTable[dstIpMatch2][4])

                  msg = of.ofp_packet_out()
                  msg.data = packet.pack()
  
                  action = of.ofp_action_output(port = self.routingTable[dstIpMatch2][3])
                  msg.actions.append(action)
                  self.connection.send(msg)
